...
bool jtag::codeBreakpointAt(unsigned int address)
{
  address /= 2;
...
bool jtag::codeBreakpointBetween(unsigned int start, unsigned int end) 
{
  start /= 2; end /= 2;
  for (int i = 0; i < numBreakpointsCode; i++)
...
void jtag::deleteAllBreakpoints(void)
{
    numBreakpointsData = numBreakpointsCode = 0;
}

bool jtag::stopAt(unsigned int address)
{
    uchar zero = 0;
...
void jtag::updateBreakpoints(bool setCodeBreakpoints)
{
