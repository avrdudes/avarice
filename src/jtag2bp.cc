...
bool jtag2::BreakpointRoom(bpType type, unsigned int length) {
    ...
    // data
    else if ((bp[bp_i].type == READ_DATA) || (bp[bp_i].type == WRITE_DATA) ||
    ...
    while (!bp[bp_i].last) {
        ...
        if (bp[bp_i].toadd || bp[bp_i].enabled) {
            if ((bp[bp_i].type == READ_DATA) || (bp[bp_i].type == WRITE_DATA) ||
                (bp[bp_i].type == ACCESS_DATA) || (bp[bp_i].type == DATA_MASK)) {
                numdata++;
